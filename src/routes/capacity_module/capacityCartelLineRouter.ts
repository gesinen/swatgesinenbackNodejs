...
class CapacityCartelLineRouter {
   ...
   * POST ('/')
   */
  public createCartelLine = () =>
    this.router.post("/", (req: Request, res: Response) => {
      const params = req.body;

      capacityCartelLineController
        .createCartelLine(
          parseInt(params.sensor_id),
          parseInt(params.user_id),
          parseInt(params.max_capacity)
        )
        .then((response) => {
          res.send(response);
        })
   ...
   * GET ('/:id')
   */
  public getCartelLines = () =>
    this.router.get("/:cartelId", (req: Request, res: Response) => {
      capacityCartelLineController
