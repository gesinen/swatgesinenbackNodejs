...
async function getCurrentCapacityAndParkingId(deviceEui) {
  let sqlGetCurrentCapacity =
    "SELECT capacity_parking.currentCapacity, capacity_parking.id as parkingId FROM `sensor_info` INNER JOIN capacity_devices ON " +
    ...
    " capacity_parking.id=capacity_type_ribbon.parkingId WHERE sensor_info.device_EUI = '" +
    deviceEui +
    "';";
...
async function getParkingCartelDeviceEuiAndLines(parkingId) {
  let sqlGetParkingCartelsIds =
    "SELECT capacity_cartel_line.cartelId FROM capacity_cartel " +
    "LEFT JOIN capacity_cartel_line ON capacity_cartel.id=capacity_cartel_line.cartelId LEFT JOIN sensor_info ON " +
    "sensor_info.id=capacity_cartel.sensorId WHERE capacity_cartel_line.parkingId=" +
    parkingId +
    ";";
  ...
  for (const element of res) {
    let sqlGetParkingCartelDeviceEuiAndLines =
      ...
      " ON sensor_info.id=capacity_cartel.sensorId WHERE capacity_cartel_line.cartelId=" +
      element.cartelId +
      " AND " +
      "capacity_cartel_line.parkingId IS NULL OR capacity_cartel_line.parkingId IS NOT NULL AND " +
      "capacity_cartel_line.cartelId=" +
      element.cartelId +
      ";";
...
let querySql =
  "SELECT capacity_parking.id as parkingId, capacity_parking.currentCapacity, capacity_parking.maxCapacity," +
...
query(querySql).then((rows) => {
  ...
  client.on("message", async function (topic, message) {
    ...
    let currentCapacityAndParkingId = await filterMqttMessage(
      deviceEUI,
      messageFormated
    );
    //console.log("currentCapacityAndParkingId", currentCapacityAndParkingId)
    let cartelDeviceEUIandLinesArray = await getParkingCartelDeviceEuiAndLines(
      currentCapacityAndParkingId.parkingId
    );
    console.log("cartelDeviceEUIandLinesArray", cartelDeviceEUIandLinesArray);
    cartelDeviceEUIandLinesArray.forEach((element, index) => {
      let setTopic =
        gatewayMac +
        "/application/2/device/" +
        cartelDeviceEUIandLinesArray[index][0].cartelDeviceEUI +
        "/tx";
      let setMessage =
        "0x1b 0x06 " +
        cartelDeviceEUIandLinesArray[index][0].currentCapacity +
        " p1m " +
        cartelDeviceEUIandLinesArray[index][1].currentCapacity +
        " p2m";
