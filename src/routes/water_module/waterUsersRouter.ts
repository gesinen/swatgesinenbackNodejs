...
class WaterUsersRouter {
     ...
     * params : nif -> user nif
     */
    public getUserByNif = () => this.router.get('/userByNif/:nif', (req: Request, res: Response) => {
        const params = req.params;

        waterDevicesController.getUserByNif(params.nif)
            .then(response => {
                res.send(response)
            })
            .catch(err => {
     ...
     */
    public getWaterUserMunicipalityIdAction = () => this.router.get('/municipality/:user_id', (req: Request, res: Response) => {
        const user_id = req.params.user_id;
     ...
     * POST ('/import')
     */
    public importFileAction = () => this.router.post('/import/:user_id/:municipality_id', (req: Request, res: Response) => {
        const user_id = req.params.user_id;
        const municipality_id = req.params.municipality_id;
        const params = req.body;
        //console.log("importFileAction -- usersRouter")
        //console.log(req.body)
        waterUsersController.importFile(params,user_id,municipality_id)
            .then( response => {
                res.send(response)
            })
            .catch( err => {
