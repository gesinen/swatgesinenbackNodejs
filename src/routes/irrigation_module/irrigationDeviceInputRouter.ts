...
class IrrigationDeviceInputRouter {
     ...
     * GET ('/information/:id')
     */
    public getIrrigationInputDeviceById = () => this.router.get('/:id', (req: Request, res: Response) => {
        const id = parseInt(req.params.id);

        irrigationDeviceInputController.getIrrigationInputDeviceById(id)
            .then((response: any) => {
                res.send(response)
            })
            .catch((err: any) => {
     ...
     */
    public storeIrrigationInputDevice = () => this.router.post('/', (req: Request, res: Response) => {
     ...
     * params user_id -> id of the user we want to get the municipality_id from
     */
    public deleteIrrigationInputDevice = () => this.router.delete('/:id', (req: Request, res: Response) => {
        const id = parseInt(req.params.id);

        irrigationDeviceInputController.deleteIrrigationInputDevice(id)
            .then((response: any) => {
                res.send(response)
            })
            .catch((err: any) => {
     ...
     * params user_id -> id of the user we want to get the municipality_id from
     */
    public updateIrrigationInputDevice = () => this.router.put('/', (req: Request, res: Response) => {
        console.log("update")
        const params = req.body;
        console.log(params)
