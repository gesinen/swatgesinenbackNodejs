...
class SensorController {
     ...
     * @return
     */
    public async createMultipleSensors(sensors: any[], provider_id: any, user_id: any): Promise<object> {
        ...
        sensors.forEach((element: any, index: any) => {
            insert_values += "('" + Utils.checkUndefined(user_id) + "','" +
                Utils.checkUndefined(element.Name) + "','" + Utils.checkUndefined(element.Description) + "','" +
                Utils.checkUndefined(provider_id) + "','" + Utils.checkUndefined(element.device_EUI) + "','" +
                Utils.checkUndefined(element.app_EUI) + "','" + Utils.checkUndefined(element.app_KEY) +
    ...
    public async addSensorObservationsFromSentilo(sensorName: any, sensorServerUrl: any, sensorProvider: any, identityKey: any) {
        return new Promise((resolve, reject) => {
    ...
    public async getSensorsByDevEUI(json_file_data: any[]) {
        return new Promise((resolve, reject) => {
            ...
            db.getConnection((err: any, conn: any) => {
                ...
                var select_query =
                    "SELECT * FROM `sensor_info` WHERE `device_EUI` IN (" + sensors_EUI.slice(0, -1) + ") ORDER BY `id` ASC;"
                //console.log(select_query)
                conn.query(select_query, (err: any, results: any) => {
                    if (err) {
                        reject({
    ...
    public async getSensorGatewayId(sensorId: any) {
        return new Promise((resolve, reject) => {
            db.getConnection((err: any, conn: any) => {
                ...
                var select_query =
                    "SELECT `gateways`.`id` FROM `sensor_gateway_pkid` INNER JOIN `gateways` ON `sensor_gateway_pkid`.`mac_number`=`gateways`.`mac` WHERE `sensor_gateway_pkid`.`sensor_id`=" + sensorId + ";"
